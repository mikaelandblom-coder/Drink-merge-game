...
- **Hitboxes**: `config/hitboxes.js` is generated by tools/hitbox-editor.html —
  ...
  (perspective vanishing row) is per-map: drag the red line in the editor,
  saved as `horizon` and applied by startGame(). Splines may cross it, but
  walls above it are unreachable in-game (balls stop at physics y=0).
...
- Score: coin count (10 coins per merge, more for higher tiers)
