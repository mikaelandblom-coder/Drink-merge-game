...
sw.js                 — Service worker: the offline copy of the game. Two
                         caches, two rules (shell = network-first, assets =
                         cache-first) — see "Playing with no network" below.
offline.js            — Registers sw.js and runs the menu's "Play offline"
                         panel (save a map to the device before a flight).
...
- **Deploy checklist**, in order:
  ...
  **`sw.js` has no script tag at all**, and GAME_VERSION is its buster:
  offline.js registers it as `sw.js?v=<GAME_VERSION>` and the worker names its
  shell cache after that query. Bumping GAME_VERSION is therefore already the
  whole step, and the preflight says so if sw.js changes without one.
